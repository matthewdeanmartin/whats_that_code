...
2. **Modernize the label system.** Keep every existing label working (backwards compatible),
   but allow many more, contemporary languages — *and* give callers a way to say "don't match
   on rare languages."
3. **Improve performance.** It has never been profiled; the suspicion is that it is slow.
   Optional native-library acceleration may help.
4. **Systematically improve match rates** with a repeatable measure-change-measure loop.
...
2. **Labels are forever.** Any label a caller can receive today (every key of
...
3. **Defaults are sacred.** Calling with the old signature must produce the *same answer* it
...
4. **Pure-Python by default.** Native acceleration is an extra (`pip install
   whats_that_code[fast]`) with a pure-Python fallback that is always present.
5. **Measure before and after.** No accuracy or perf change lands without a number from the
...
2. **Data-consistency test.** `test/test_fast/test_label_registry.py`: assert every key in
...
3. **Characterization corpus + golden test.** Assemble ~50–100 short, license-clean snippets
...
4. **Public-API snapshot.** Add a `make api-snapshot` target using `griffe` (already a dev dep)
...
5. **Fix the README example** (`result == ["python"]` → `result == "python"`) and any doc that
...
2. **Manifest & licensing:** `corpus/MANIFEST.json` recording source, URL, commit/snapshot date,
   license, and file count per language. Update `docs/Data.md` to describe the rebuild process
   and dead links.
3. **Held-out split:** deterministic train/dev/test split (seeded) so accuracy numbers are
   stable and not contaminated by tuning.
4. **Evaluation harness `scripts/evaluate.py` (the core deliverable):** runs
...
5. **Revive the slow test:** rewrite `test/test_slow/test_classify_all.py` to consume the new
...
6. **Record the baseline number** in `spec/spec.md` or `eval_report.json` so Phases 2–4 have a
   "before."
...
2. **Add new languages additively.** Expand the registry with modern languages (e.g. typescript
...
3. **Rare-language suppression (the requested feature).** Add an opt-in knob. Recommended shape
...
4. **Confidence signal (optional but recommended).** Consider a sibling function that returns
...
2. **Pure-Python wins (no new deps):** precompile/curry regexes once at import; short-circuit
...
3. **Optional native acceleration (extra, never required):** e.g. fast keyword scanning via
...
4. **Micro-benchmark in CI (advisory):** keep `scripts/profile.py` runnable; optionally a
...
2. **Targeted, low-risk improvements**, each validated by the harness, e.g.:
...
3. **Guard every change** with: characterization test green (defaults sacred), eval harness
   shows overall ↑ and **no per-language regression beyond a small tolerance**, api-check green.
4. **Track a scoreboard:** append each experiment's eval delta to a log
   (`spec/accuracy_log.md`) so progress is visible and regressions are attributable.
5. **Consider a "is this even code?" gate** (the old Release 3.0 TODO) only as an additive,
